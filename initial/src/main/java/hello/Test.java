...
public class Test{
	...
	public void readUtilFile(Model model) {
		...
		try {
			...
			if (!setObj.utilityFilePath.toString().endsWith("xlsx")) {
				...
				w1 = new XSSFWorkbook(classLoader.getResource(setObj.utilityFilePath).getFile());
			}
	...
	public void findExecTestscript(String TestSuite, String TestScript,
		...
		try{
			...
			fs = new FileInputStream(new File(getClass().getClassLoader().getResource(TestSuite).getFile()));
...

	{
		...
		fs1 = new FileInputStream(new File(classLoader.getResource(scriptPath).getFile()));
		...
		try{
			...
			LOGGER.log(Level.SEVERE, "Error Occured, while reading TestScript File in execKeywordScript  "+e.getMessage() );
				...
				try {
					fs2 = new FileInputStream(new File(classLoader.getResource(ORPath).getFile()));
